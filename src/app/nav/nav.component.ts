...
export class NavComponent implements OnInit{
  counter: number
  constructor(iconRegistry: MatIconRegistry, sanitizer: DomSanitizer, private _productService: ProductService, private _router: Router) {
    // Note that we provide the icon here as a string literal here due to a limitation in
    // Stackblitz. If you want to provide the icon from a URL, you can use:
    // `iconRegistry.addSvgIcon('thumbs-up', sanitizer.bypassSecurityTrustResourceUrl('icon.svg'));`
    iconRegistry.addSvgIconLiteral('shopping-cart', sanitizer.bypassSecurityTrustHtml(SHOPPING_CART_ICON));
    this.counter = 0;
  }
  ...
  ngOnInit(): void {
    this._productService.tokenProviderObservable$.subscribe((data)=>{
      ...
      console.log('Cart badge updated:', this.counter);
    });
  }
}
